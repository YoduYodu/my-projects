...
import edu.usfca.protobuf.Bid.BidRequestLight;
import edu.usfca.protobuf.Profile.Data1;
import edu.usfca.protobuf.Profile.Data2;
import edu.usfca.protobuf.Profile.KeyOnly1;
import edu.usfca.protobuf.Profile.KeyOnly2;
import edu.usfca.protobuf.Profile.ValueOnly1;
import edu.usfca.protobuf.Profile.ValueOnly2;

/**
 * .
 */
public class L29PartialDeserialization {
  ...
  static void helperPrint(String prefix, Message msg) {
    ...
    for (int i = 0; i < arr.length; i++) {
      ...
      if ((i + 1) % 50 == 0 || i + 1 == arr.length)
        System.out.println();
    }
    System.out.println("---------------------------------------------------------> ");
  }

  // ------------------ Revisiting example from L28.
  @Test
  public void testWithData2() throws InvalidProtocolBufferException {
    Data2 data2 = Data2.newBuilder().addMyKey("C").addMyVal(70) //
        .addMyKey("S").addMyVal(80).build();

    helperPrint("data2 ", data2); // 10 bytes.

    // ------------------------------------------
    final byte[] data2serialized = data2.toByteArray();
    KeyOnly2 key2 = KeyOnly2.parseFrom(data2serialized);
    ValueOnly2 val2 = ValueOnly2.parseFrom(data2serialized);
    helperPrint("key2 ", key2); // 6 bytes.
    helperPrint("val2 ", val2); // 4 bytes.
    assertEquals(key2.getMyKeyList(), data2.getMyKeyList());
    assertEquals(val2.getMyValList(), data2.getMyValList());
  }

  @Test
  public void testWithData1() throws InvalidProtocolBufferException {
    // TODO: Ungraded homework:
    // Figure out how to define "KeyOnly1" and "ValueOnly1" messages in profile.proto,
    // so as to have the same "effect" as in the unit test above.
    // "KeyOnly1" should partially deserialize byte[] of "Data1" message (just the "keys").
    Data1 data1 = Data1.newBuilder().putMyMap("C", 70).putMyMap("S", 80).build();

    helperPrint("data1 ", data1); // 14 bytes.

    // ------------------------------------------
    final byte[] data1serialized = data1.toByteArray();
    KeyOnly1 key1 = KeyOnly1.parseFrom(data1serialized);
    ValueOnly1 val1 = ValueOnly1.parseFrom(data1serialized);
    helperPrint("key1 ", key1); // likely to be 10 bytes.
    helperPrint("val2 ", val1); // likely to be 8 bytes.
    // TODO: Add assertions to compare the results.
  }
  // ------------------

  static byte[] getSampleBidRequest() {
    BidRequest.Builder br = BidRequest.newBuilder();
    // Useful information that we wish to extract for our pipeline:
    br.setId("usf");
    ...
    br.addAllBadv(Arrays.asList("Lorem", "ipsum", "dolor", "sit", "amet,", "consectetur", "adipiscing", "elit,", "sed",
        ...
        "est", "laborum."));
    br.addAllBapp(Arrays.asList("Velit", "sed", "ullamcorper", "morbi", "tincidunt", "ornare.", "At", "auctor", "urna",
        "nunc", "id", "cursus", "metus", "aliquam", "eleifend", "mi.", "Sit", "amet", "facilisis", "magna", "etiam",
        "tempor", "orci", "eu.", "Praesent", "elementum", "facilisis", "leo", "vel", "fringilla", "est", "ullamcorper",
        "eget", "nulla.", "Ac", "turpis", "egestas", "integer", "eget", "aliquet", "nibh", "praesent.", "A", "diam",
        "sollicitudin", "tempor", "id", "eu", "nisl", "nunc", "mi", "ipsum.", "Sed", "viverra", "ipsum", "nunc",
        "aliquet", "bibendum", "enim", "facilisis.", "Nulla", "aliquet", "porttitor", "lacus", "luctus.", "Sed",
        "faucibus", "turpis", "in", "eu", "mi.", "Turpis", "egestas", "sed", "tempus", "urna", "et", "pharetra",
        "pharetra.", "In", "nisl", "nisi", "scelerisque", "eu", "ultrices", "vitae", "auctor."));
    br.addAllBcat(Arrays.asList("Morbi", "tincidunt", "augue", "interdum", "velit", "euismod", "in", "pellentesque.",
        "Ut", "etiam", "sit", "amet", "nisl", "purus", "in", "mollis", "nunc", "sed.", "Laoreet", "sit", "amet",
        "cursus", "sit", "amet", "dictum.", "Sapien", "et", "ligula", "ullamcorper", "malesuada", "proin", "libero",
        "nunc", "consequat", "interdum.", "Orci", "porta", "non", "pulvinar", "neque.", "Egestas", "dui", "id",
        "ornare", "arcu", "odio", "ut", "sem", "nulla.", "In", "fermentum", "posuere", "urna", "nec", "tincidunt.",
        "Risus", "sed", "vulputate", "odio", "ut", "enim", "blandit.", "Felis", "eget", "nunc", "lobortis", "mattis",
        "aliquam", "faucibus", "purus", "in.", "Amet", "est", "placerat", "in", "egestas.", "Nec", "sagittis",
        "aliquam", "malesuada", "bibendum", "arcu", "vitae.", "Cursus", "eget", "nunc", "scelerisque", "viverra",
        "mauris", "in", "aliquam", "sem", "fringilla.", "Morbi", "tristique", "senectus", "et", "netus", "et",
        "malesuada", "fames", "ac.", "Risus", "pretium", "quam", "vulputate", "dignissim", "suspendisse", "in", "est",
        "ante.", "Massa", "placerat", "duis", "ultricies", "lacus", "sed", "turpis.", "Facilisi", "morbi", "tempus",
        "iaculis", "urna", "id", "volutpat", "lacus.", "Massa", "tempor", "nec", "feugiat", "nisl."));
    br.addAllCur(Arrays.asList("6bc2af68-ee7d-4afc-ac5c-d8727fed2309", "e657bc60-9575-499a-bd46-e4b48364e45c",
  ...
  // Fastest: Serialized data (byte[]) contains only the data we need.
  @Test
  public void testBaseline() throws InvalidProtocolBufferException {
    final byte[] data = BidRequest.parseFrom(getSampleBidRequest()).toBuilder().clearBadv().clearBapp().clearBcat()
        .clearCur().build().toByteArray();
    helperPrint("BidRequest: ", BidRequest.parseFrom(data));

    Instant start = Instant.now();
    long total = 0;
    for (int i = 0; i < 1000000; i++) {
      BidRequest br = BidRequest.parseFrom(data);
      total += br.getId().length();
      assertEquals("ios", br.getDevice().getOs());
      assertEquals("usf", br.getId());
      if ((i + 1) % 50000 == 0) {
        System.out.format("Elapsed time: %s (after %6d iterations)\n",
            DurationFormatUtils.formatDurationHMS(Instant.now().getMillis() - start.getMillis()), i + 1);
      }
    }
    System.out.format("Total = %d (this makes your compiler and IDE happy)\n", total);
  }

  // Slowest: Serialized data (byte[]) contains excessive data we do not really need,
  // which causes a HUGE slowdown in de-serializing it.
  @Test
  public void testSlow() throws InvalidProtocolBufferException {
    final byte[] data = getSampleBidRequest();
    helperPrint("BidRequest: ", BidRequest.parseFrom(data));
  ...
  // Moderate: Serialized data (byte[]) contains excessive data we do not really need,
  // but we are de-serializing only parts of it, so this is much faster than "testSlow()".
  @Test
  public void testHacky() throws InvalidProtocolBufferException {
    final byte[] data = getSampleBidRequest();
    helperPrint("BidRequestLight: ", BidRequestLight.parseFrom(data));

    Instant start = Instant.now();
    long total = 0;
    for (int i = 0; i < 1000000; i++) {
      BidRequestLight br = BidRequestLight.parseFrom(data);
      total += br.getId().length();
      assertEquals("ios", br.getDevice().getOs());
      assertEquals("usf", br.getId());
      if ((i + 1) % 50000 == 0) {
