...
/**
 * A {@link DoFn} that writes elements to files with names deterministically derived from the lower and upper bounds of
 * their key (an {@link IntervalWindow}).
 *
 * <p>
...
public class WriteOneFilePerWindow extends PTransform<PCollection<String>, PDone> {
  ...
  /**
   * A {@link FilenamePolicy} produces a base file name for a write based on metadata about the data being written. This
   * always includes the shard number and the total number of shards. For windowed writes, it also includes the window
   * and pane index (a sequence number assigned to each trigger firing).
   */
  public static class PerWindowFiles extends FilenamePolicy {
    ...
    public ResourceId windowedFilename(int shardNumber, int numShards, BoundedWindow window, PaneInfo paneInfo,
      ...
      String filename = String.format("%s-%s-of-%s%s", filenamePrefixForWindow(intervalWindow), shardNumber, numShards,
